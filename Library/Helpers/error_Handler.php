...
Class error_handler{
    ...
    public function is_already_associated($marker, $student,$type,$Mark_ID){
        ...
        if($Mark_ID != -1){
            if($row['id_mark'] != null && $Mark_ID != $row['id_mark']){
                echo "failed IAA1<br>";
    ...
    public function validate_mark($M1,$M2,$M3,$marker, $student,$sem_type, $Mark_ID){
        ...
        if($this->is_already_associated($marker, $student,$sem_type,$Mark_ID)){
            $boolean_flag = false;
        }
    ...
    public function new_marker_check($MFN,$MLN){
        $boolean_flag = true;
        // intial test to check the form information
        if($this->check_name($MFN)){
            $this->error_string .= "Invalid first name <br>";
            $boolean_flag = false;
        }
        if($this->check_name($MLN)){
            $this->error_string .= "Invalid last name <br>";
            $boolean_flag = false;
        }
        
        if($boolean_flag == true){  // passed the intial test
            $query = "select * from markers where marker_first_name=\"".$MFN ."\",marker_last_name=\"".$MLN ."\"";
            $result = $this->database_connection->query_Database($query);
            if($result ==false){
