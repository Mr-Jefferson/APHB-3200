/* 
 * Overall marker final marks view for table on marker page.
 */
USE seminar_marks;
...
SELECT  CONCAT(marker_first_name, ' ', marker_last_name) AS marker_name,
        ...
        ROUND(AVG(mark_1)*0.1 + AVG(mark_2)*0.1 + AVG(mark_3)*0.8,2)*10 AS overall_average,
        COUNT(mark_1) AS number_of_marks,
        ROUND(MIN(mark_1*0.1 + mark_2*0.1 + mark_3*0.8),2)*10 AS minimum_mark,
        ROUND(MAX(mark_1*0.1 + mark_2*0.1 + mark_3*0.8),2)*10 AS maximum_mark,
        ROUND(STDDEV(mark_1*0.1 + mark_2*0.1 + mark_3*0.8),2)*10 AS standard_deviation,
		marks.id_marker,
        students.cohort,
        students.semester
