...
SELECT  CONCAT(marker_first_name, ' ', marker_last_name) AS marker_name,
        ...
        TRUNCATE(AVG(mark_1)*0.1 + AVG(mark_2)*0.1 + AVG(mark_3)*0.8,2) AS overall_average,
        COUNT(mark_1) AS number_of_marks,
        MIN(mark_1*0.1 + mark_2*0.1 + mark_3*0.8) AS minimum_mark,
        MAX(mark_1*0.1 + mark_2*0.1 + mark_3*0.8) AS maximum_mark,
        STDDEV(mark_1*0.1 + mark_2*0.1 + mark_3*0.8) AS standard_deviation,
		marks.id_marker,
        students.cohort,
        students.semester
